...
describe('Alter Q ROI geometry', () => {
  it('clamps rectangles and supports move plus all eight resize handles', () => {
    const origin = {x: 0.25, y: 0.25, w: 0.2, h: 0.2};
    const handles: AlterQRoiHandle[] = ['move', 'nw', 'ne', 'sw', 'se', 'n', 's', 'w', 'e'];
    for (const handle of handles) {
      const transformed = transformAlterQRoi(origin, handle, 0.05, -0.04);
      const clamped = clampAlterQRoi(transformed, {minW: 0.02, minH: 0.02});
  ...
  it('converts normalized ROI to logical coordinates on a scaled monitor', () => {
    expect(alterQRoiToPhysicalGeometry(
