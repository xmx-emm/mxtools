...
import {
  getApexVideoconfigFolderPath,
  openFolderDetached as openFolderDetachedCmd,
...
export async function openFolderDetached(folderPath: string): Promise<void> {
  await openFolderDetachedCmd({path: folderPath});
}
...
export async function openApexVideoConfigFolder(): Promise<void> {
  const path = await getApexVideoconfigFolderPath();
  await openPath(path);
}
