...
export type ApexQRoiHandle = 'move' | 'nw' | 'ne' | 'sw' | 'se' | 'n' | 's' | 'e' | 'w';

export interface ApexQRoiBounds {
  minW: number;
  minH: number;
  maxW?: number;
  maxH?: number;
...
export function clampApexQRoi(roi: ApexQRoi, bounds: ApexQRoiBounds): ApexQRoi {
...
export function transformApexQRoi(
  origin: ApexQRoi,
  handle: ApexQRoiHandle,
  dx: number,
  dy: number,
): ApexQRoi {
  let next: ApexQRoi;
...
export function apexQRoiToPhysicalGeometry(
  roi: ApexQRoi,
  monitor: {
    width: number;
    height: number;
