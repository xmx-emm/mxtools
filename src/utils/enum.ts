type EnumStringKey<T> = Extract<keyof T, string>;

export function getEnumKeyValuePairs<T extends Record<string, string | number>>(enumObj: T): Array<{
  key: EnumStringKey<T>,
  value: T[keyof T]
}> {
  const keys = Object.keys(enumObj) as Array<EnumStringKey<T>>;
...
export function fromValueGetKey<T extends Record<string, string | number>>(
  enumObj: T,
  value: string | number,
): EnumStringKey<T> | null {
