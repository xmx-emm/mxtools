...
import ApexLobbyFpsPreset from '@/components/game/apex/preset/ApexLobbyFpsPreset.vue';
...
function translateApexLaunchOptionText(key?: string) {
  if (!key) return '';
  if (key.startsWith('apexLaunchOptions.')) return t(key);
  return key;
}

function bilingualSearchText(key?: string): string {
  if (!key) return '';
  if (!key.startsWith('apexLaunchOptions.')) return key;
