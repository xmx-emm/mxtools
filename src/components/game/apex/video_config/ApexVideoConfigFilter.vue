...
const localSearch = ref(apex_store.video_filter_search ?? '');
let searchTimer: ReturnType<typeof setTimeout> | null = null;

watch(
  () => apex_store.video_filter_search,
  (v) => {
    const next = v ?? '';
...
watch(localSearch, (v) => {
  ...
  searchTimer = setTimeout(() => {
    searchTimer = null;
    if ((apex_store.video_filter_search ?? '') !== v) {
      apex_store.video_filter_search = v;
    }
  }, 120);
});
...
<template>
  <div class="d-flex flex-row align-center flex-nowrap apex-launch-filters">
    <v-text-field
      ...
      hide-details
      clearable
      :placeholder="t('apex.videoFilterSearchPlaceholder')"
      :aria-label="t('apex.videoFilterSearchPlaceholder')"
      prepend-inner-icon="mdi-magnify"
    />
    <v-spacer/>
    <v-btn-toggle
      v-model="apex_store.video_filter_type"
