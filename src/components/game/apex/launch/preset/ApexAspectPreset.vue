...
import {ASPECT_LETTERBOX_THRESHOLD} from '@/data/presets/apex_quick_preset.ts';
...
const all_aspects: AspectPreset[] = [
  ...
  { title: '16:10 (1.60)', value: 1.60 },
  { title: '3:2 (1.50)', value: 1.50 },
...
function apply_aspect(value: number) {
  ...
  apex_store.mat_letterbox_aspect_goal = value;
  apex_store.mat_letterbox_aspect_threshold = ASPECT_LETTERBOX_THRESHOLD;
}

const selected_aspect_title = computed(() => {
  const target = sorted_aspects.value.find((item) => item.value === apex_store.mat_letterbox_aspect_min && item.value === apex_store.mat_letterbox_aspect_goal);
  ...
  <v-menu closeOnBack :open-on-click="false" openOnHover openDelay="150" closeDelay="100">
    ...
    <v-list>
      <v-list-item
        ...
        :key="item.title"
        :title="item.title"
        :active="item.value === apex_store.mat_letterbox_aspect_min && item.value === apex_store.mat_letterbox_aspect_goal"
