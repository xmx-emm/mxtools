...
import ApexManualDownloadMilesLanguage from '@/components/game/apex/ApexManualDownloadMilesLanguage.vue';
...
<template>
  <v-col class="page-content d-flex flex-column h-100">
    <div>
      <v-alert
        v-if="steam_store.is_steam_running"
        icon="mdi-steam"
        density="compact"
        :text="t('apex.steamRunningTip')"
        :title="t('apex.steamRunning')"
        type="error"
      />
    ...
    <ApexSelectLaunchOptions
        style="flex:1;"/>
    <v-btn-group style="max-height: 40px;">
      <ApexApexCopyButton/>
    ...
    <v-dialog v-model="apex_store.tip_dialog">
      <component :is="apex_store.tip_view" @click="apex_store.closeTip()" @contextmenu="apex_store.closeTip()"
                 class="not_select"/>
    </v-dialog>
