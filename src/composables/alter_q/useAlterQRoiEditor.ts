...
import {
  ...
  alterQTestOcr,
  type AlterQOcrParseResult,
} from '@/ipc/commands.ts';
import type {AlterQRoi} from '@/types/alter_q.ts';
import {DEFAULT_PING_ROI, DEFAULT_SHOWPOS_ROI, loadAlterQPrefs} from '@/types/alter_q.ts';
import {normalizeAlterQFolder} from '@/utils/alter_q.ts';
...
export type AlterQRoiEditorProps = {
  modelValue: boolean;
  folder: string;
  showposRoi: AlterQRoi;
  pingRoi: AlterQRoi;
};
export type AlterQRoiEditorEmits = {
  'update:modelValue': [value: boolean];
  confirm: [payload: {showposRoi: AlterQRoi; pingRoi: AlterQRoi}];
};
type Emit = <K extends keyof AlterQRoiEditorEmits>(event: K, ...args: AlterQRoiEditorEmits[K]) => void;

export function useAlterQRoiEditor(props: Readonly<AlterQRoiEditorProps>, emit: Emit) {
  ...
  const draftShowpos = reactive<AlterQRoi>({...DEFAULT_SHOWPOS_ROI});
  const draftPing = reactive<AlterQRoi>({...DEFAULT_PING_ROI});
  const imagePath = ref('');
  ...
  const verifyResult = ref<AlterQOcrParseResult | null>(null);
  ...
  function clampRoi(r: AlterQRoi, forKind: RoiKind = 'showpos'): AlterQRoi {
    const minW = forKind === 'showpos' ? SHOWPOS_MIN_W : PING_MIN_W;
    const minH = forKind === 'showpos' ? SHOWPOS_MIN_H : PING_MIN_H;
    return clampAlterQRoi(r, {minW, minH});
  }
  
  function assignRoi(target: AlterQRoi, next: AlterQRoi, forKind: RoiKind) {
  ...
  async function loadPreview(path: string) {
    ...
    try {
      const normalized = await normalizeAlterQFolder(path);
      if (!screenshotPicker.isPreviewCurrent(gen)) return;
      // 校准主图：原图不缩小，避免放大后发糊
      const src = await alterQScreenshotPreview({path: normalized, maxEdge: 0});
      if (!screenshotPicker.isPreviewCurrent(gen)) return;
  ...
  async function runOcrVerify() {
    if (!imagePath.value) {
      toast.warning(t('apex.alterQ.calibrateNoImage'));
      return;
    }
    ...
    try {
      const result = await alterQTestOcr({
        path: requestPath,
        showposRoi: requestShowposRoi,
        pingRoi: requestPingRoi,
        kind: active,
        engine: loadAlterQPrefs().ocrEngine ?? 'auto',
      });
      ...
      if (active === 'showpos') {
        if (result.alpha != null) {
          toast.success(t('apex.alterQ.calibrateVerifyOkShowpos', {alpha: result.alpha.toFixed(2)}));
        } else {
          toast.warning(t('apex.alterQ.calibrateVerifyPartialShowpos'));
        }
      } else if (result.distanceM != null) {
        toast.success(
          t('apex.alterQ.calibrateVerifyOkPing', {distance: result.distanceM.toFixed(1)}),
        );
      } else {
        toast.warning(t('apex.alterQ.calibrateVerifyPartialPing'));
      }
    } catch (e) {
      if (requestGen !== verifyGen || imagePath.value !== requestPath) return;
      verifiedKinds[active] = false;
  ...
  async function refreshRecent() {
    ...
    try {
      const paths = await alterQListRecentScreenshots({
        folder: await normalizeAlterQFolder(props.folder),
        limit: 12,
      });
  ...
  function confirm() {
    if (!allRegionsVerified.value) {
      toast.warning(t('apex.alterQ.calibrateVerifyRequired'));
      return;
    }
    emit('confirm', {
