...
import type {ApexQOverlayPlacement, ApexQRoi} from '@/types/apex_q.ts';
import {
...
export type ApexQOverlayPlacementEditorProps = {
  modelValue: boolean;
  folder: string;
  ...
  placement?: ApexQOverlayPlacement | null;
};
export type ApexQOverlayPlacementEditorEmits = {
...
type Emit = <K extends keyof ApexQOverlayPlacementEditorEmits>(event: K, ...args: ApexQOverlayPlacementEditorEmits[K]) => void;

export function useApexQOverlayPlacementEditor(props: Readonly<ApexQOverlayPlacementEditorProps>, emit: Emit) {
  ...
  const draft = reactive<ApexQRoi>({x: 0.7, y: 0.75, w: 0.12, h: 0.1});
  ...
  const aspectMismatchText = computed(() => t('apex.apexQ.overlayAspectMismatch', {
  ...
  const monitorItems = computed(() => monitors.value.map((monitor, index) => ({
    title: `${index + 1}. ${monitor.name || t('apex.apexQ.overlayMonitorFallback', {index: index + 1})} - ${Math.round(monitor.size.width)} x ${Math.round(monitor.size.height)} @ ${Math.round(monitor.position.x)}, ${Math.round(monitor.position.y)}`,
    value: index,
  ...
  const selectedMonitorText = computed(() => {
    ...
    return `${monitor.name || t('apex.apexQ.overlayMonitor')} · ${Math.round(monitor.size.width)} x ${Math.round(monitor.size.height)}`;
  });
  ...
  function clampRoi(r: ApexQRoi, sw = screenW.value, sh = screenH.value): ApexQRoi {
    ...
    return clampApexQRoi(r, {minW, minH, maxW, maxH});
  }

  function assignRoi(next: ApexQRoi) {
  ...
  function roiToGeometry(r: ApexQRoi, sw: number, sh: number) {
    const monitor = selectedMonitor.value;
    const scale = monitor?.scaleFactor || 1;
    const origin = monitor?.position.toLogical(scale) ?? {x: 0, y: 0};
    ...
    const placement: ApexQOverlayPlacement = {
      version: 2,
  ...
  async function loadDisplaySize(options: {silent?: boolean; preserveSelection?: boolean} = {}) {
    ...
    try {
      ...
      const selected = preserved
        ?? selectApexQOverlayMonitor(monitors.value, placementPrefs(), fallback);
  ...
  async function loadPreview(path: string) {
    ...
    try {
      const src = await apexQScreenshotPreview({path, maxEdge: 0});
  ...
  async function refreshRecent() {
    ...
    try {
      const folder = props.folder.trim()
        ? await normalizeApexQFolder(props.folder)
        : '';
      if (!folder) {
        recentPaths.value = [];
        recentThumbs.value = [];
      ...
      const paths = await apexQListRecentScreenshots({folder, limit: 12});
  ...
  function resetRect() {
    const monitor = selectedMonitor.value;
    assignRoi(defaultApexQOverlayPlacement(monitor ? {
      name: monitor.name,
  ...
  watch(
    ...
    () => {
      ...
      const selected = selectApexQOverlayMonitor(
        monitors.value,
        placementPrefs(),
