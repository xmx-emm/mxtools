...
代码侧预检（2026-07-29）：`npm run lint`、87 项前端测试、
`cargo fmt --check`、严格 Clippy 和 76 项 Rust 测试均已通过；另有 1 项需要管理员权限的
SMB 集成测试保持忽略。下列「代码预检」仅说明实现已到位，**仍需真机点过才算通过**。

旧的未压缩 0.0.6 便携版已确认能够启动并创建主窗口；新的缓存式自解压便携版仍需
确认首次解压与第二次缓存复用。自动化工具无法捕获当前无边框 Tauri 窗口
（`0x80004002`），因此没有用盲点方式代替人工验收，也没有自动修改真实 Steam、输入法、
RDP、防火墙或端口转发状态。
