...
状态：0.0.8 已接入签名发布、Gitee 国内清单和 GitHub 回退
更新日期：2026-09-12

当前实现与日常操作以 [ONLINE_UPDATE_RELEASE.md](ONLINE_UPDATE_RELEASE.md) 为准。
公钥已内置，私钥保存在仓库外并配置为 GitHub Secrets。下文保留最初方案的
设计背景，其中分支地址、构建命令和发布步骤不再作为当前操作说明。
