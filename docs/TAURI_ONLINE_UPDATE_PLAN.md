...
状态：GitHub 在线更新已实现；Gitee 镜像与回退尚未实现
更新日期：2026-09-08

当前源码通过 `src-tauri/src/app_update.rs` 检查 GitHub Releases，验证签名后安装；
普通安装版需在构建时配置公钥，便携版走手动更新，已打包商店应用走商店更新。
Windows 更新退出前会恢复后台硬件状态，并保留 Tauri 的退出清理。
设置页已接入检查、进度及错误状态。当前本地环境没有配置更新签名密钥，
因此不能把候选构建当作在线更新验收通过。下文国内镜像和发布流水线仍是待实施方案。
