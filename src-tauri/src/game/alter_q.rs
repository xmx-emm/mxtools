...
pub struct AlterQCaptureResult {
...
fn capture_error_result(screenshot_path: String, error: impl Into<String>) -> AlterQCaptureResult {
    AlterQCaptureResult {
        screenshot_path,
        alpha: None,
        ang_yaw: None,
...
#[tauri::command]
pub async fn alter_q_ocr_available() -> IpcResult<bool> {
    blocking_cmd(|| Ok(ocr_available()))
        .await
        .map_err(alter_q_error)
}

#[tauri::command]
pub async fn alter_q_ocr_status() -> IpcResult<crate::game::alter_q_rapid_ocr::OcrStatus> {
...
#[tauri::command]
pub async fn alter_q_ocr_download(app: tauri::AppHandle) -> IpcResult<()> {
    alter_q_ocr_download::download_ocr_pack(app)
        .await
        .map_err(|message| IpcError::new("alter_q.ocr_download_failed", message))
}

#[tauri::command]
pub async fn alter_q_ocr_delete() -> IpcResult<()> {
    blocking_cmd(alter_q_ocr_download::delete_ocr_pack)
        .await
        .map_err(|message| IpcError::new("alter_q.ocr_delete_failed", message))
}

#[tauri::command]
pub async fn alter_q_normalize_path(path: String) -> IpcResult<String> {
    Ok(normalize_windows_path(&path))
}

#[tauri::command]
pub async fn alter_q_list_steam_screenshot_dirs() -> IpcResult<Vec<SteamScreenshotDirCandidate>> {
...
#[tauri::command]
pub async fn alter_q_suggested_screenshot_dir() -> IpcResult<Option<String>> {
    blocking_cmd(|| {
        Ok(
...
#[tauri::command]
pub async fn alter_q_latest_screenshot(folder: String) -> IpcResult<String> {
    blocking_cmd(move || {
...
#[tauri::command]
pub async fn alter_q_list_recent_screenshots(folder: String, limit: u32) -> IpcResult<Vec<String>> {
    blocking_cmd(move || {
...
#[tauri::command]
pub async fn alter_q_screenshot_preview(path: String, max_edge: u32) -> IpcResult<String> {
    use base64::Engine;
...
#[tauri::command]
pub async fn alter_q_test_ocr(
    path: String,
    showpos_roi: RoiRect,
    ping_roi: RoiRect,
    kind: Option<String>,
    engine: Option<String>,
) -> IpcResult<crate::game::alter_q_ocr::OcrParseResult> {
    blocking_cmd(move || {
        let path = folder_path(&path);
        if !path.is_file() {
            return Err(format!("Screenshot not found: {}", path.display()));
...
#[tauri::command]
pub async fn alter_q_compute_theta(r: f64, alpha: f64) -> IpcResult<ThetaResult> {
    if !r.is_finite() || r <= 0.0 {
        return Err(IpcError::new(
            "alter_q.invalid_distance",
            "Distance r must be a positive number",
        ));
    }
    if !alpha.is_finite() {
        return Err(IpcError::new(
            "alter_q.invalid_alpha",
...
#[tauri::command]
pub async fn alter_q_default_rois() -> IpcResult<(RoiRect, RoiRect)> {
    Ok((default_showpos_roi(), default_ping_roi()))
}

#[tauri::command]
pub async fn alter_q_from_latest_screenshot(
    folder: String,
    delay_ms: u64,
    showpos_roi: RoiRect,
    ping_roi: RoiRect,
...
#[tauri::command]
pub async fn alter_q_open_ocr_settings() -> IpcResult<()> {
