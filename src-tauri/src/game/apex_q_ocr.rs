...
pub fn ocr_available() -> bool {
    crate::game::apex_q_rapid_ocr::rapid_ready() || try_create_ocr_engine().is_ok()
}
...
#[cfg(windows)]
pub fn ocr_status() -> crate::game::apex_q_rapid_ocr::OcrStatus {
    let rapid = crate::game::apex_q_rapid_ocr::rapid_ready();
    ...
    crate::game::apex_q_rapid_ocr::OcrStatus {
        rapid_ready: rapid,
        win_ready: win,
        install_dir: crate::game::apex_q_ocr_download::status_install_dir(),
...
pub fn parse_screenshot_kind(
    ...
    let rapid_ok = want_rapid && crate::game::apex_q_rapid_ocr::rapid_ready();
    ...
    if prefer == "rapid" && !rapid_ok {
        return Err(
            "OCR_UNAVAILABLE: RapidOCR pack not installed. Download it in APEX Q → OCR / Settings."
                .into(),
        );
    }
    ...
    if !rapid_ok && win_engine.is_none() {
        return Err(
      "OCR_UNAVAILABLE: Download RapidOCR pack in the APEX Q window, or install English OCR language pack (en-US)."
        .into(),
    );
    }
...
fn ocr_showpos_rapid(img: &DynamicImage, roi: &RoiRect) -> Result<ShowposReading, String> {
    ...
    for variant in variants {
        let Ok(raw) = crate::game::apex_q_rapid_ocr::recognize_rgba(&variant) else {
            continue;
        };
