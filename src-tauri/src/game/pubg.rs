...
#[tauri::command]
pub async fn get_pubg_launch_option(id: usize) -> Result<String, String> {
    blocking_cmd(move || get_pubg_launch_options_by_steam_user_id(id)).await
}

#[tauri::command]
pub async fn set_pubg_launch_option(id: usize, launch_option: String) -> Result<(), String> {
    blocking_cmd(move || set_pubg_launch_options_by_steam_user_id(id, &launch_option)).await
}

/// 检查“禁用开场动画”是否已生效(Movies 是否已重命名为 Movies_disabled)。
#[tauri::command]
pub async fn check_pubg_skip_intro_movies_disabled() -> Result<bool, String> {
    blocking_cmd(check_skip_intro_movies_disabled).await
}

/// 设置“禁用开场动画”状态(Movies <-> Movies_disabled 可逆重命名)。
#[tauri::command]
pub async fn set_pubg_skip_intro_movies_disabled(disabled: bool) -> Result<(), String> {
    blocking_cmd(move || set_skip_intro_movies_disabled(disabled)).await
}

/// PUBG 客户端日志目录(%LOCALAPPDATA%\\TslGame\\Saved\\Logs),不存在则创建
#[tauri::command]
pub async fn get_pubg_logs_folder_path() -> Result<String, String> {
    blocking_cmd(|| {
        #[cfg(windows)]
        {
...
#[tauri::command]
pub async fn open_folder_detached(path: String) -> Result<(), String> {
