...
/// Apex owns the schema version. A preset-only file created after reset has
/// no version and must be regenerated by the game, not locked as a valid config.
pub(crate) fn video_config_is_initialized(values: &HashMap<String, String>) -> bool {
    values.get("setting.configversion").is_some_and(|value| {
        value.bytes().all(|byte| byte.is_ascii_digit())
            && value.parse::<u32>().is_ok_and(|version| version > 0)
    })
}
