...
pub(crate) fn patch_video_config_sync(updates: &HashMap<String, String>) -> Result<(), String> {
    ...
    read_initialized_video_config_sync()?;
    patch_apex_videoconfig(updates).map(|_| ())
}

/// J57 rejects versions below 7, and versions 7-9 read legacy map-detail/SSAO
/// keys. Our current field catalog needs version >=10 within signed int32.
/// This is a compatibility guard, not proof that every field is present.
/// See docs/APEX_VIDEO_CONFIG_FORMAT.md; do not merely stamp a partial file.
pub(crate) fn video_config_is_initialized(values: &HashMap<String, String>) -> bool {
    values.get("setting.configversion").is_some_and(|value| {
        value.bytes().all(|byte| byte.is_ascii_digit())
            && value.parse::<i32>().is_ok_and(|version| version >= 10)
    })
}

pub(crate) fn read_initialized_video_config_sync() -> Result<HashMap<String, String>, String> {
    let values = read_video_config_sync()?;
    if !video_config_is_initialized(&values) {
        return Err("apex.videoConfigNeedsGeneration".into());
    }
    Ok(values)
}
...
pub async fn start_apex_ea(ea_user_id: String) -> IpcResult<()> {
    blocking_cmd(move || {
        ...
        get_apex_audio_folder_path_by_platform(Some("ea"), Some(&ea_user_id))
            .ok_or("apex.milesDlEa.eaNotInstalled")?;
        let desktop = super::apex_language_download_ea::ea_desktop_exe()?;
        let directory = desktop.parent().ok_or("apex.milesDlEa.eaNotFound")?;
        let launcher = directory.join("EALauncher.exe");
        if !launcher.is_file() {
...
pub async fn set_apex_video_config(
    ...
    blocking_cmd(move || {
        validate_video_updates(&updates)?;
        let _guard = lock_history()?;
        let current = read_initialized_video_config_sync()?;
        let changed = updates
            .iter()
...
pub async fn set_apex_videoconfig_readonly(locked: bool) -> IpcResult<()> {
    blocking_cmd(move || {
        ...
        set_videoconfig_readonly(locked)
    })
    .await
    .map_err(apex_error)
}
...
pub async fn set_apex_config_file(
    ...
    blocking_cmd(move || {
        ...
        if matches!(kind, ApexConfigFileKind::VideoConfig) {
            return patch_video_config_sync(&updates);
        }
...
mod tests {
    ...
    include!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../tests/rust/src-tauri/game/apex_video_initialization.rs"
    ));
}
